...
export function BookingFlow({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-ink/50 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div className="relative max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-3xl bg-bone p-6 shadow-lift sm:rounded-3xl sm:p-8">
        <button
          onClick={onClose}
          ...
          {(["recap", "pay", "done"] as Step[]).map((s, i) => (
            <span
              ...
              className={cn(
                ...
                ["recap", "pay", "done"].indexOf(step) >= i ? "bg-ember" : "bg-line",
              )}
            />
          ))}
        </div>
        ...
        {step === "recap" && (
          <div>
            <p className="eyebrow text-peri-deep">Réservation</p>
            ...
            <div className="mt-5 flex items-end justify-between">
              ...
              <div className="flex items-baseline gap-2">
                <span className="font-display text-3xl font-medium text-ember-deep">{formatEuro(price)}</span>
            ...
            <Button variant="ember" size="lg" className="mt-5 w-full" onClick={() => setStep("pay")}>
        ...
        {step === "pay" && (
          <div>
            <p className="eyebrow text-peri-deep">Paiement sécurisé</p>
        ...
        {step === "done" && (
          <div className="text-center">
            <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-peri-tint text-peri-deep">
              <Check className="h-7 w-7" />
            </span>
            <h2 className="mt-4 font-display text-2xl text-ink">C&apos;est réservé !</h2>
            <p className="mt-1 text-sm text-ink-soft">
              Présentez ce code à l&apos;accueil de {offer.gym}.
            </p>

            <div className="mx-auto mt-6 w-fit rounded-2xl bg-paper p-5 ring-1 ring-line">
              <Qr value={ref} />
              <p className="mt-3 font-mono text-sm tracking-wider text-ink">{ref}</p>
            </div>

            <div className="mt-5 space-y-2 rounded-2xl bg-secondary/60 p-4 text-left text-sm">
              <p className="flex items-center gap-2 text-ink"><Clock className="h-4 w-4 text-peri-deep" /> {slotLabel(offer.startsInHours)} · {offer.durationMin} min</p>
              <p className="flex items-center gap-2 text-ink"><MapPin className="h-4 w-4 text-peri-deep" /> {offer.address}</p>
            </div>
...
function Row({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 text-sm">
      <span className="mt-0.5 text-peri-deep">{icon}</span>
