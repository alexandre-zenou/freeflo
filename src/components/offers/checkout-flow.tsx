...
import { bookingRef } from "@/lib/account";
import { cartTotal, type CartItem } from "@/lib/cart";

type Step = "recap" | "pay" | "done";

/**
 * Tunnel de paiement du PANIER (ex `booking-flow.tsx`, qui ne traitait qu'une
 * offre à la fois). On règle tout le panier en une fois : récapitulatif, carte
 * de démonstration, puis la carte de confirmation bordeaux de la maquette
 * cliente, qui porte maintenant une ligne par cours.
 *
 * Aucun pourcentage nulle part (retour client 08/2026) : prix bloqué et prix
 * plein barré, rien d'autre.
 */
export function CheckoutFlow({
  ...
  open: boolean;
  onClose: () => void;
  /** Appelé une fois le paiement (simulé) passé, pour inscrire les réservations. */
  onConfirmed?: () => void;
}) {
  const t = useT();
  ...
  /* Le panier se vide à la confirmation : on repart du récapitulatif à la
     réouverture, sinon le tunnel rouvrirait sur une confirmation périmée. */
  useEffect(() => {
  ...
  const lines = items
    ...
    .filter((l): l is { item: CartItem; offer: NonNullable<typeof l.offer> } => Boolean(l.offer));
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-ink/50 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div className="relative max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-3xl bg-cream p-6 shadow-lift sm:rounded-3xl sm:p-8">
        ...
        {step === "recap" && (
          <div>
            <p className="eyebrow text-brand">{t("Votre panier", "Your cart")}</p>
            <h2 className="mt-2 font-display text-2xl text-ink">
              {plural
                ? t(`${lines.length} places à confirmer`, `${lines.length} spots to confirm`)
                : t("Une place à confirmer", "One spot to confirm")}
            </h2>

            <div className="mt-5 space-y-4">
              {lines.map(({ item, offer }) => (
                <div key={item.offerId} className="rounded-2xl bg-paper p-4 ring-1 ring-line">
                  <p className="font-bold text-ink">{t(offer.title, offer.titleEn)}</p>
                  ...
                  <div className="mt-3 space-y-2">
                    <Row
                      ...
                      value={`${slotLabel(offer.startsInHours, locale)}, ${offer.durationMin} min`}
                    />
                    <Row
                    ...
                    <Row
                      ...
                      label={t("Annulation", "Cancellation")}
                      value={t("Gratuite jusqu'à 6 h avant", "Free up to 6 h before")}
                    />
                  </div>
                  <p className="mt-3 flex items-baseline justify-between border-t border-line pt-3">
                    <span className="text-sm text-ink-soft">{t("Prix bloqué", "Locked price")}</span>
                    <span className="flex items-baseline gap-2">
                      <span className="font-display text-lg font-bold tabular-nums text-brand-deep">
            ...
            <div className="mt-5 flex items-end justify-between">
              <span className="text-sm text-ink-soft">{t("Total", "Total")}</span>
              <span className="font-display text-3xl font-medium text-brand-deep">
                {formatEuro(total)}
              </span>
            </div>

            <Button variant="gold" size="lg" className="mt-5 w-full" onClick={() => setStep("pay")}>
              {t("Payer", "Pay")} {formatEuro(total)}
            </Button>
            <p className="mt-3 text-center text-xs text-ink-soft">
              {t(
                "Places garanties dès le paiement confirmé. Aucune adhésion.",
                "Spots guaranteed once payment clears. No membership.",
              )}
            </p>
          </div>
        )}
        ...
        {step === "pay" && (
          <div>
            ...
            <h2 className="mt-2 font-display text-2xl text-ink">
              {formatEuro(total)} {t("à régler", "to pay")}
            </h2>
            <p className="mt-1 flex items-center gap-1.5 text-xs text-ink-soft">
              <Lock className="h-3 w-3" />{" "}
              {t(
                "Démo : paiement sécurisé en production. N'entrez pas de vraie carte.",
                "Demo: secure payment in production. Do not enter a real card.",
              )}
            </p>

            <div className="mt-5 space-y-3">
              <Field label={t("Nom sur la carte", "Name on card")} placeholder="Thomas Durand" />
            ...
            <Button variant="solid" size="lg" className="mt-6 w-full" onClick={pay} disabled={processing}>
              {processing ? t("Traitement…", "Processing…") : `${t("Payer", "Pay")} ${formatEuro(total)}`}
            </Button>
            <button
          ...
          <div className="text-center">
            ...
            <div className="mt-6 rounded-3xl bg-brand-deep p-6 text-left text-white">
              <h3 className="text-center text-lg font-bold">
                {t("Confirmation de réservation", "Booking confirmation")}
              </h3>

              <div className="mt-5 space-y-4">
                {lines.map(({ item, offer }) => (
                  <div key={item.offerId}>
                    <p className="text-lg font-bold">{t(offer.title, offer.titleEn)}</p>
                    ...
                    <p className="mt-1 font-mono text-xs text-white/70">
                      ...
                      {bookingRef(offer.id, offer.basePrice, offer.placesLeft)}
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-5 rounded-2xl bg-secondary/60 p-4 text-left text-sm">
              <p className="flex items-center gap-2 text-ink">
                ...
                {t(
                  ...
                  "Confirm your name at the front desk at the time booked.",
                )}
              </p>
            </div>
            ...
            >
              {t("Voir mes réservations", "View my bookings")}
            </Link>
            <Link
              href="/offres"
              className="mt-2 inline-block w-full text-center text-sm text-ink-soft underline underline-offset-4 hover:text-ink"
            >
