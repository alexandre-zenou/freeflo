...
export function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      ...
      {sprint && sprintPrice && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl bg-brand/10 px-5 py-4 ring-1 ring-brand/25">
          ...
          <p className="min-w-0 flex-1 text-sm text-ink">
            ...
            <strong className="tabular-nums text-brand-deep">−{sprintPrice.discountPct}%</strong>.
            Chaque place vendue vaut mieux que zéro.
          </p>
          <button
            onClick={onGoToOffers}
            className="flex items-center gap-1 text-sm font-medium text-brand-deep transition-colors hover:text-ink"
          >
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-5 xl:grid-cols-4">
        {kpis.map((k) => (
          <div key={k.label} className="border-t border-ink/20 pt-4">
            <p className="serif-display text-3xl text-ink">{k.value}</p>
            <p className="mt-1 flex items-center gap-1.5 text-xs text-ink-soft">
              <k.icon className="h-3.5 w-3.5 text-pro-accent" />
              {k.label} <span className="ml-auto text-pro-accent">{k.delta}</span>
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <div className="rounded-2xl bg-white p-6 ring-1 ring-line">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-pro-accent">+18%</span>
          </div>
          <div className="mt-6 flex h-40 items-end gap-3">
            {revenueChart.map((v, i) => (
              <div key={i} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                <div
                  className="w-full max-w-9 rounded-t-lg bg-gradient-to-t from-brand to-gold"
                  style={{ height: `${v}%` }}
                />
        ...
        <div className="rounded-2xl bg-ink p-6 text-cream">
          ...
          <div className="mt-5 space-y-2 border-t border-white/10 pt-4 text-sm">
            ...
            <p className="flex justify-between font-medium"><span>Net à recevoir</span><span className="text-pro-accent">126 €</span></p>
          </div>
        </div>
      </div>
